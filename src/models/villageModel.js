...
const Village = {
  ...
  async insertVillagesFromExcel(data, project_id, type) {
    ...
    const [existingVillages] = await db.query(
      `SELECT id, village_name, tahasil, district, thana_name_no 
       FROM villages 
      ...
      if (existingVillage) {
        if (existingVillage.id) {
          const shouldUpdateThana =
            thanaNo &&
            (!existingVillage.thana_name_no ||
              existingVillage.thana_name_no.toString().trim() === "");
          ...
          await db.query(
            `UPDATE villages 
             SET thana_name_no = COALESCE(?, thana_name_no),
          ...
          if (shouldUpdateThana) existingVillage.thana_name_no = thanaNo;
  ...
  async insertVillageForManualPlot(plot, project_id, type) {
    ...
    const thanaNo = normalize(plot.thana_name_no);
    ...
    if (existing.length > 0) {
      if (thanaNo) {
        await db.query(
          `UPDATE villages
           SET thana_name_no = ?, updated_at = NOW()
           WHERE id = ?
             AND (thana_name_no IS NULL OR TRIM(thana_name_no) = '')`,
